...
# --- PARÁMETROS DE PROTECCIÓN ACTIVA ---
BE_THRESHOLD = 1.5  # Si el precio avanza 1.5x ATR, movemos a Break-Even
TS_DISTANCE = 1.5   # El Stop Loss perseguirá al precio manteniendo 1.5x ATR de distancia
...
def gestionar_proteccion_activa(posiciones, magic_number):
    """Aplica Break-Even y Trailing Stop dinámico a las posiciones abiertas"""
    import MetaTrader5 as mt5
    ...
    for p in posiciones:
        ...
        symbol = p.symbol
        # Necesitamos el ATR actual para que la protección sea proporcional a la volatilidad
        ...
        if p.type == 0:  
            distancia_ganancia = p.price_current - p.price_open
            
            # 1. Trigger de Break-Even (Asegurar entrada)
            if distancia_ganancia > (atr * BE_THRESHOLD) and p.sl < p.price_open:
                nuevo_sl = p.price_open + (atr * 0.1) # Entrada + un pequeño margen
                modificar = True
                agregar_log(f"🛡️ BE Activado: {symbol}")

            # 2. Trailing Stop (Perseguir ganancia)
        ...
        # --- LÓGICA PARA VENTAS (SELL) ---
            ...
            distancia_ganancia = p.price_open - p.price_current
            
            # 1. Trigger de Break-Even
            if distancia_ganancia > (atr * BE_THRESHOLD) and p.sl > p.price_open:
        ...
        if modificar:
            ...
            res = mt5.order_send(request)
            if res.retcode != mt5.TRADE_RETCODE_DONE:
                print(f"Error modificando SL en {symbol}: {res.comment}")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # ==========================================
            # NUEVO: PROTEGER OPERACIONES ANTES DE ANALIZAR NUEVAS
            # ==========================================
            if posiciones:
                gestionar_proteccion_activa(posiciones, MAGIC_NUMBER)
